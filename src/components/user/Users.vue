<template>
  <div>
    ...
    <el-card>
      ...
      >
        <div>
          <p>当前用户的名称：{{ userInfo.username }}</p>
          <p>当前用户的角色：{{ userInfo.role_name }}</p>
          <p>
            分配新角色
            <el-select
              v-model="newRoleValueSelected"
              placeholder="请选择"
            >
              <el-option
...
export default {
  ...
  methods: {
    ...
    async saveRoleInfo () {
      if (!this.newRoleValueSelected) {
        return this.$message.error('请选择要分配的角色！')
      }
      const { data: res } = await this.$http.put(
        `users/${this.userInfo.id}/role`,
        { rid: this.newRoleValueSelected }
      )
      if (res.meta.status !== 200) {
        return this.$message.error('用户角分配失败')
      }
      this.$message.success('用户角分配成功')
      this.getUserList()
